...
import ex.talosdx.companyregister.ui.validators.EmployeeValidatorMapperImpl;
...
public class EmployeePopUpControllerImpl extends AbstractPopUpController<Employee>
{
     ...
     */
    public EmployeePopUpControllerImpl(GenericDAO<Employee> employeeDAO, GenericDAO<Company> companyDAO, MainPage ui)
    {
    ...
    public void displayAddItem(Window window)
    {
        ...
        {
            LazyUtils.clearFields(new Field[]{employeeField, dateField, emailField, companyBox});
            window.close();
        });
        ...
        saveButton.addClickListener(event1 ->
        {
            ...
            {
                Employee employee = new Employee(employeeField.getValue(), LazyUtils.toLocalDate(dateField.getValue()), emailField.getValue(), (Integer) companyBox.getValue());
            ...
            {
                LazyUtils.showErrorWithFields(new Field[]{employeeField, dateField, emailField, companyBox});
            }
        });
    ...
    public void displayEditItem(Window window, Employee employee)
    {
        ...
        cancelButton.addClickListener(event1 ->
        {
            LazyUtils.clearAction(ownContent, new Component[]{employeeField, dateField, emailField, companyBox, saveButton, cancelButton});
            window.close();
        });
        saveButton.addClickListener(event1 ->
        {
            if (employeeField.isValid() && dateField.isValid() && emailField.isValid() && companyBox.isValid())
            {
                ...
                employee.setBirthday(LazyUtils.toLocalDate(dateField.getValue()));
            ...
            else LazyUtils.showErrorWithFields(new Field[]{employeeField, dateField, emailField, companyBox});
    ...
    public void displayDeleteItem(Window window, Employee employee)
    {
        ...
        cancelButton.addClickListener(event1 ->
        {
            ...
            LazyUtils.clearAction(ownContent, new Component[]{employeeField, dateField, emailField, companyBox, deleteButton, cancelButton});
            window.close();
        });
