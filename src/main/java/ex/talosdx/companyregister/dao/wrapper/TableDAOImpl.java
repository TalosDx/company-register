...
import static ex.talosdx.companyregister.dao.DBConnector.sqlExecuteForInt;
import static ex.talosdx.companyregister.dao.DBConnector.sqlUpdate;
...
public class TableDAOImpl implements AbstractTableDAO
{
    ...
    @Override
    public <T extends RowMapper> Object selectColumnsByColumn(String tableName, String[] nameColumns, String column, Object obj, T mapper)
    {
        MapSqlParameterSource parameterSource = new MapSqlParameterSource();
        parameterSource.addValue(column, column);
        return DBConnector.sqlExecuteForObject(getSelectRequestByColumn(tableName, nameColumns, column), new MapSqlParameterSource(column, obj), mapper);    }


    public String getSelectRequestByColumn(String tableName, String[] nameColumns, String column)
    {
        String sql = "SELECT ";
        String str;
        for (int i=0; i<nameColumns.length; i++)
        {
            str = i == nameColumns.length-1 ? " from " + tableName + " where " + column +" = :" + column : ", ";
            sql += nameColumns[i] + str;
        }
        return sql;
    }
